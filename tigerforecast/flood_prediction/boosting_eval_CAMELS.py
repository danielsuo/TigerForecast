...
# method_boosted.initialize([method_LSTM, method_ar], loss=batched_mse, reg=REG)
# print("reg = " + str(REG))
results_boosted = []
pred_boosted = []
       ...
       for i, site in enumerate(all_sites[:1]):
              ...
              if i == 0:
                      ...
                      for i in range(len(site0_yhats_LSTM)):
                              site0_LSTM_residuals.append(site0_ys[i] - site0_yhats_LSTM[i])
                              site0_boosted_residuals.append(site0_ys[i] - site0_yhats_boosted[i])
                      # plt.plot(site0_yhats_LSTM, label="LSTM")
                      # plt.plot(site0_yhats_boosted, label="boosted")
                      # plt.plot(site0_ys, label="ground truth")
                      plt.plot(site0_LSTM_residuals, label="LSTM residuals")
                      plt.plot(site0_boosted_residuals, label="boosted residuals")
                      plt.legend()
                      plt.title("LSTM vs boosted residuals")
