...
#@profile
def test_all_methods(problem_name, filename, verbose = 2, lr_tuning = False):
	exp = Experiment()
	exp.initialize(timesteps = 1, n_runs = 3, verbose = verbose)
	...
	exp.add_method('LSTM', {'optimizer' : OGD})
	exp.add_all_method_variants('LSTM', lr_tuning = lr_tuning, include_boosting = False)
	exp.scoreboard()
	# exp.graph(size = 6)
...
if __name__ == "__main__":
	...
	parser.add_argument("--verbose", default=2, type=str, help="select level of verbosity")
